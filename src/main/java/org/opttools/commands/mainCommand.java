...
public class mainCommand implements CommandExecutor, TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(" ");
            sender.sendMessage(colorSet.CText("&7[&3JoinOpt&7] &fv3.0.1(3)"));
            sender.sendMessage(colorSet.CText("&7⨠ &7A plugin for edit &fPlayerJoin/Exit &7messages"));
            sender.sendMessage(colorSet.CText("&7 "));
            sender.sendMessage(colorSet.CText("&7⨠ &7Author: &fDApples_"));
            sender.sendMessage(colorSet.CText("&7 "));
            //Website
            ...
            github.setText(colorSet.CText("&7[&f⛏ &fGithub&7]"));
        ...
        if (args[0].equalsIgnoreCase("reload")) {
            if (sender instanceof Player) {
                sender.sendMessage(colorSet.CText(pluginMessage.Tips + "&fReloaded"));
                ((Player) sender).playSound(((Player) sender).getLocation(), Sound.BLOCK_NOTE_BLOCK_PLING, 3, 2);
                loadVariable.load();
